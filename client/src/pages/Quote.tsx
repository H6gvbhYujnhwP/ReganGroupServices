  ...
  if (step === 4) return (
    <div className="pt-16 md:pt-20 min-h-screen flex items-center justify-center" style={{ background: "#f7fdf2" }}>
      <div className="text-center max-w-md mx-auto px-4 py-20">
        ...
        <p className="text-base mb-6" style={{ color: "#5a7a3a" }}>
          Thank you, {form.name}! We've received your quote request and will be in touch within 2 hours.
        </p>
        <div className="rounded-2xl p-6 text-left space-y-3 mb-8" style={{ background: "#eef7e6", border: "1.5px solid #c8e6a0" }}>
          <div className="text-sm"><span className="font-semibold" style={{ color: "#1a2e0a" }}>Services: </span><span style={{ color: "#5a7a3a" }}>{form.services.join(", ")}</span></div>
          <div className="text-sm"><span className="font-semibold" style={{ color: "#1a2e0a" }}>Property: </span><span style={{ color: "#5a7a3a" }}>{form.propertyType}</span></div>
          <div className="text-sm"><span className="font-semibold" style={{ color: "#1a2e0a" }}>Urgency: </span><span style={{ color: "#5a7a3a" }}>{form.urgency}</span></div>
        </div>
      ...
      <section className="py-12" style={{ background: "linear-gradient(135deg, #eef7e6 0%, #d4edba 100%)" }}>
        <div className="container text-center">
          <h1 className="text-5xl md:text-6xl font-extrabold mb-3" style={{ fontFamily: "'Barlow Condensed', sans-serif", color: "#1a2e0a" }}>
            GET YOUR <span style={{ color: "#5B9E1F" }}>FREE QUOTE</span>
          </h1>
          <p className="text-base" style={{ color: "#4a6a2a" }}>No obligation. Competitive pricing. Fast response.</p>
        </div>
      </section>
        ...
        <div className="container max-w-3xl mx-auto">
            ...
            {([1, 2, 3] as const).map((s) => (
              <div key={s} className="flex items-center gap-2 flex-1">
                <div className="flex items-center justify-center w-9 h-9 rounded-full font-bold text-sm flex-shrink-0 transition-all"
                  style={{ background: step >= s ? "#5B9E1F" : "#eef7e6", color: step >= s ? "#fff" : "#5a7a3a" }}>
                  {step > s ? <CheckCircle size={18} /> : s}
                </div>
                <div className="text-xs font-semibold hidden sm:block" style={{ color: step >= s ? "#5B9E1F" : "#a8c88a" }}>
                  {["Service Details", "Property Info", "Your Details"][s - 1]}
                </div>
                {s < 3 && <div className="flex-1 h-0.5 rounded" style={{ background: step > s ? "#5B9E1F" : "#e8f5d8" }} />}
              </div>
            ))}
          </div>
          ...
          <div className="rounded-2xl p-8 shadow-lg" style={{ background: "#f7fdf2", border: "1.5px solid #e8f5d8" }}>
            ...
            {step === 1 && (
              <div>
                ...
                <p className="text-sm mb-6" style={{ color: "#5a7a3a" }}>Select all that apply</p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                  {services.map((s) => (
                    <button key={s} type="button" onClick={() => toggleService(s)}
                      className="p-3 rounded-xl text-sm font-semibold text-center transition-all hover:scale-105"
                      style={{
                        background: form.services.includes(s) ? "#5B9E1F" : "#fff",
                        color: form.services.includes(s) ? "#fff" : "#2d4a1a",
                        border: `1.5px solid ${form.services.includes(s) ? "#5B9E1F" : "#c8e6a0"}`,
                      }}>
                      {s}
                    </button>
                  ))}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {urgencies.map((u) => (
                      <button key={u} type="button" onClick={() => setForm(f => ({ ...f, urgency: u }))}
                        ...
                        style={{
                          background: form.urgency === u ? "#eef7e6" : "#fff",
                          color: "#2d4a1a",
                          border: `1.5px solid ${form.urgency === u ? "#5B9E1F" : "#c8e6a0"}`,
                        }}>
                        {u}
                      </button>
                    ))}
                ...
                <div className="mb-6">
                  ...
                  <textarea rows={4} className="w-full px-4 py-3 rounded-xl border text-sm outline-none resize-none" style={{ borderColor: "#c8e6a0", background: "#fff", color: "#1a2e0a" }}
                ...
                <button onClick={() => setStep(2)} disabled={form.services.length === 0}
                  ...
                  style={{ background: "#5B9E1F", color: "#fff", fontFamily: "'Barlow Condensed', sans-serif", fontSize: "1.1rem" }}>
            ...
            {step === 2 && (
              <div>
                ...
                <p className="text-sm mb-6" style={{ color: "#5a7a3a" }}>This helps us provide an accurate quote</p>
                <div className="mb-5">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {propertyTypes.map((p) => (
                      <button key={p} type="button" onClick={() => setForm(f => ({ ...f, propertyType: p }))}
                        ...
                        style={{
                          background: form.propertyType === p ? "#5B9E1F" : "#fff",
                          color: form.propertyType === p ? "#fff" : "#2d4a1a",
                          border: `1.5px solid ${form.propertyType === p ? "#5B9E1F" : "#c8e6a0"}`,
                        }}>
                        {p}
                      </button>
                    ))}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                  <div>
                    ...
                    <input className="w-full px-4 py-3 rounded-xl border text-sm outline-none" style={{ borderColor: "#c8e6a0", background: "#fff", color: "#1a2e0a" }}
                ...
                <div className="flex gap-3">
                  <button onClick={() => setStep(1)} className="flex-1 py-4 rounded-xl font-bold text-base border-2 transition-all hover:scale-[1.02]"
                    style={{ borderColor: "#5B9E1F", color: "#5B9E1F", fontFamily: "'Barlow Condensed', sans-serif", fontSize: "1rem" }}>
            ...
            {step === 3 && (
              <div>
                ...
                <p className="text-sm mb-6" style={{ color: "#5a7a3a" }}>We'll use these to send your quote</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                  <div>
                    <label className="block text-xs font-semibold mb-1.5" style={{ color: "#2d4a1a" }}>Full Name *</label>
                    <input required className="w-full px-4 py-3 rounded-xl border text-sm outline-none" style={{ borderColor: "#c8e6a0", background: "#fff", color: "#1a2e0a" }}
                ...
                <div className="mb-4">
                  ...
                  <input required type="email" className="w-full px-4 py-3 rounded-xl border text-sm outline-none" style={{ borderColor: "#c8e6a0", background: "#fff", color: "#1a2e0a" }}
                ...
                <div className="flex items-center gap-3 mb-6 p-4 rounded-xl" style={{ background: "#eef7e6" }}>
                  <input type="checkbox" id="newsletter" checked={form.newsletter}
                    onChange={e => setForm(f => ({ ...f, newsletter: e.target.checked }))}
                    className="w-4 h-4 rounded" style={{ accentColor: "#5B9E1F" }} />
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-6">
            {["Free, No-Obligation Quote", "Response Within 2 Hours", "Competitive Pricing", "Fully Insured"].map((t) => (
              <div key={t} className="flex items-center gap-2 text-sm" style={{ color: "#5a7a3a" }}>
